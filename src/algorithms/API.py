...
class LaplacianPyramid:
    def __init__(
        self,
        fusion_kernel_size=6,
        pyramid_num_levels=8,
        use_pyqt=True,
        use_gpu=False,
        selected_gpu_id=0,
    ):
    ...
    def toggle_cpu_gpu(self):
        ...
        if self.use_pyqt:
            self.Algorithm.toggle_cpu_gpu(
                ...
                int(
                    settings.globalVars["QSettings"].value("computing/selected_gpu_id")
                ),
            )
        else:
            self.Algorithm.toggle_cpu_gpu(self.use_gpu, int(self.selected_gpu_id))
